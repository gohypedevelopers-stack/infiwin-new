...
export const Navbar = ({ isSolid = false }: { isSolid?: boolean }) => {
  ...
  const showSolid = isSolid || isScrolled || activeMenu !== null || isSubPage;
    ...
    <>
      ...
      >
        <div className="max-w-[1600px] mx-auto flex justify-between items-center relative">
          {/* Logo */}
          <Link href="/" className="flex items-center shrink-0 mr-8 xl:mr-12">
            <img
          ...
          <div className="hidden lg:flex items-center justify-center gap-3 xl:gap-8 font-sans flex-1">
            {NAV_ITEMS.map((item) => {
              ...
              return (
                ...
                >
                  <Link
                    href={item.href}
                    className={`text-[13px] xl:text-[14px] font-semibold tracking-wide transition-colors duration-300 hover:text-brand-primary flex items-center gap-1 xl:gap-1.5 relative py-1 ${showSolid ? 'text-brand-dark' : 'text-white'}`}
                  >
                    {item.name}
                    {hasDropdown && (
                      <ChevronDown
          ...
          <div className="hidden lg:flex items-center gap-3 xl:gap-6 shrink-0 justify-end relative ml-8 xl:ml-12">
            {/* Myntra-Style Interactive Search Bar */}
            <div ref={searchContainerRef} className="relative w-full max-w-[130px] xl:max-w-[160px] 2xl:max-w-[240px]">
              <div
              ...
              <AnimatePresence>
                {isSearchFocused && (
                  <motion.div
                    ...
                    className="absolute top-[110%] left-1/2 -translate-x-1/2 w-[300px] bg-white border border-neutral-100 shadow-2xl rounded-2xl p-4 overflow-hidden z-[120]"
                  >
            ...
            <Link
              href="/contact"
              className={`px-4 py-2 xl:px-5 xl:py-2 2xl:px-6 2xl:py-2.5 rounded-full text-[11px] xl:text-xs font-semibold tracking-wide transition-all duration-300 shadow-sm shrink-0 ${showSolid
                ? 'bg-brand-primary text-white hover:bg-brand-dark'
                : 'bg-white text-brand-dark hover:bg-brand-primary hover:text-white'
                }`}
            >
              Get <span className="hidden 2xl:inline">Free </span>Quote
            </Link>
          </div>
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div
            ...
            className="fixed inset-0 bg-white z-[150] lg:hidden overflow-y-auto overscroll-none"
            style={{ WebkitOverflowScrolling: 'touch' }}
          >
            <div className="min-h-[100dvh] flex flex-col p-6 md:p-8">
              {/* Header */}
              <div className="flex justify-between items-center shrink-0 mb-8 pb-6 border-b border-neutral-100">
                <img
                  src="https://www.infiwindow.com/images/logo1.png"
                  alt="INFIWIN"
              ...
              {/* Mobile Link list: Home, Products, Applications, Concepts, About Us */}
              ...
              {NAV_ITEMS.map((item) => {
                ...
                let subItems: any[] = [];
                if (item.name === "Products") subItems = PRODUCTS;
                else if (item.name === "Applications") subItems = APPLICATIONS;
                else if (item.name === "Concepts") subItems = CONCEPTS;
                  ...
                  <div key={item.name} className="flex flex-col border-b border-neutral-100 pb-3">
                    <div className="flex items-center justify-between min-h-[48px]">
                      ...
                      {hasDropdown && (
                        <button
                          ...
                          className="h-10 w-10 flex items-center justify-center bg-neutral-50 rounded-full text-brand-dark ml-2 active:bg-neutral-200 transition-colors"
                          aria-label={`Toggle ${item.name} sub-menu`}
                        >
                          <ChevronDown size={16} className={`transition-transform duration-300 ${isExpanded ? 'rotate-180 text-brand-primary' : ''}`} />
                        </button>
                      )}
                    </div>
                    ...
                    <AnimatePresence>
                      {hasDropdown && isExpanded && (
                        ...
                        >
                          <ul className="flex flex-col gap-3 mt-3 pl-4 border-l-[3px] border-neutral-100">
                            {subItems.map((sub) => (
                              <li key={sub.name} className="flex items-start gap-3">
                                <div className="h-1.5 w-1.5 rounded-full bg-brand-primary/60 mt-1.5 shrink-0" />
                                <Link
                                  href={sub.href}
                                  onClick={() => setIsMobileMenuOpen(false)}
                                  className="text-sm font-medium text-neutral-600 hover:text-brand-primary py-0.5 leading-tight"
                                >
                                  {sub.name}
                                </Link>
                              </li>
              ...
              {/* Mobile Drawer CTAs */}
              <div className="mt-12 flex flex-col gap-3 border-t border-neutral-100 pt-8 shrink-0 pb-12">
                <a
                  href="/contact"
                  onClick={() => setIsMobileMenuOpen(false)}
